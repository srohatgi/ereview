...
- (void)registerDevice:(CDVInvokedUrlCommand *)command {
	...
	if ([[options objectForKey:@"badge"] boolValue]) {
		notificationTypes |= UIRemoteNotificationTypeBadge;
	}
	if ([[options objectForKey:@"sound"] boolValue]) {
		notificationTypes |= UIRemoteNotificationTypeSound;
	}
	if ([[options objectForKey:@"alert"] boolValue]) {
...
- (void)getDeviceUniqueIdentifier:(CDVInvokedUrlCommand *)command {
    ...
    if ([[UIDevice currentDevice] respondsToSelector:@selector(identifierForVendor)]) {
        ...
        // Here I use UIDevice-with-UniqueIdentifier-for-iOS-5
